...
const PredictionContent = ({ myRanking, otherRanking, live }) => {
  console.log("CONTENT", myRanking, otherRanking, live);
  return myRanking.ranking.map((team, index) => {
    ...
    return (
      <tr key={index}>
        <td className={`compare-table-ranking ${positionColor}`}>
          {index + 1}
        </td>
        <TeamCell className={positionColor} team={live[index]} />
        <TeamCell className={positionColor} team={team} />
        <PointCell className={pointsToColorStyle(myPoints)} points={myPoints} />
        {otherRanking && (
          <TeamCell
            className={positionColor}
            team={otherRanking.ranking[index]}
          />
        )}
        {otherRanking && (
          <PointCell
            className={pointsToColorStyle(theirPoints)}
            points={theirPoints}
          />
        )}
      </tr>
