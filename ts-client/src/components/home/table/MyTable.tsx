        ...
        <Table>
          <TableHead>
            <TableRow sx={{ height: 30 }}>
              ...
              <TableCell align="left" sx={{ fontSize: '0.9rem', padding: 1 }}>
                Live Standings
              </TableCell>
              {otherTable && (
                <>
                  <TableCell align="left" sx={{ fontSize: '0.9rem', padding: 1 }}>
                    {otherTable.username}'s Standings
                  </TableCell>
                  <TableCell sx={{ fontSize: '0.9rem', padding: 1 }}>Points</TableCell>
                </>
              )}
              <TableCell align="left" sx={{ fontSize: '0.9rem', padding: 1 }}>
                {myTable.username}'s Standings
              </TableCell>
              <TableCell sx={{ fontSize: '0.9rem', padding: 1 }}>Points</TableCell>
            </TableRow>
          </TableHead>
            ...
            {liveTable.ranking.map((team, index) => {
              ...
              return (
                <TableRow key={team} sx={{ height: 30 }}>
                  <TableCell sx={{ width: 30, fontSize: '0.9rem', padding: 1 }}>
                    {index + 1}
                  </TableCell>
                  <TableCell align="left" sx={{ fontSize: '0.9rem', padding: 1, ...teamNameStyle }}>
                    {isMobileDevice() ? (
                      <Box
                        m={'auto'}
                        display={'flex'}
                  ...
                  {otherTable && (
                    <>
                      <TableCell
                        align="left"
                        sx={{ fontSize: '0.9rem', padding: 1, ...teamNameStyle }}
                      >
                        {isMobileDevice() ? (
                          <Box
                            m={'auto'}
                          ...
                          otherTable.ranking[index]
                        )}
                      </TableCell>
                      <TableCell
                        sx={{ ...getOffsetStyle(otherOffset), fontSize: '0.9rem', padding: 1 }}
                      >
                  ...
                  <TableCell align="left" sx={{ fontSize: '0.9rem', padding: 1, ...teamNameStyle }}>
                    {isMobileDevice() ? (
                      <Box
                        ...
                        component={'img'}
                        width={25}
                        src={liveTable.logoUrls[liveTable.ranking.indexOf(myTable.ranking[index])]}
                      />
                    ) : (
                      myTable.ranking[index]
                    )}
                  </TableCell>
                  <TableCell sx={{ ...getOffsetStyle(myOffset), fontSize: '0.9rem', padding: 1 }}>
